...
Spectrum path_tracing(const Scene &scene, pcg32_state &rng, Ray& ray, RayDifferential& ray_diff, Real eta_scale = Real(1), Spectrum current_path_throughput = fromRGB(Vector3f(1,1,1)), int depth = 0) {
    ...
    //Indirect Light radiance
    {
        ...
        current_path_throughput = current_path_throughput * (G * brdf_indirect) / (pdf_indirect * rr_prob);
...
//     if (depth == 0 ) {
//         current_path_throughput = fromRGB(Vector3{1, 1, 1});
...
//         std::optional<IntersectPoint> bsdf_vertex = intersect(scene, bsdf_ray);
//
...
//         radiance += path_tracing(scene, rng, ray, ray_diff, eta_scale, current_path_throughput, depth + 1);
//
//     return radiance;
// }
