...
Spectrum vol_path_tracing_1(const Scene &scene,
    ...
    std::optional<IntersectPoint> vertex_ = intersect(scene, ray, ray_diff);
    if (!vertex_) {
        // Assume Non Environment Map
        return make_zero_spectrum();
    }
    IntersectPoint vertex = *vertex_;
...
int update_medium(const IntersectPoint& isect, const Ray& ray, int current_medium_id){
...
Spectrum vol_path_tracing_3(const Scene &scene,
    ...
    while(true){
        bool scatter = false;
        std::optional<IntersectPoint> isect_ = intersect(scene, ray, ray_diff);
...
inline Spectrum next_event_estimation(
    ...
    const IntersectPoint* isect = nullptr // 如果是 surface，需要传入 isect
) {
    ...
    while (true) {
        ...
        std::optional<IntersectPoint> shadow_isect_ = intersect(scene, shadow_ray, shadow_ray_diff);
        if (shadow_isect_) {
            const IntersectPoint& shadow_isect = *shadow_isect_;
