...
{
public:
    MeshTriangle(std::string name, const Vector3f* verts, const uint32_t* vertsIndex, const uint32_t& numTris, const Vector2f* st, Material *mt = new Material()):MeshTriangle( verts, vertsIndex, numTris, st, mt) {
        this->m_name = name;
    }
...
//                                   face_vertices[2], mt);
    ...
    MeshTriangle(std::string name, const std::string& filename, Vector3f offset, Material *mt = new Material()):MeshTriangle(filename, offset, mt) {
        this->m_name = name;
    }
