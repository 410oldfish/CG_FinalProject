...
{   
    ...
    constexpr float RussianRoulette = 1.f;
    ...
    Vector3f normal_hit = inter.normal; // Normal at the hit point
    ...
    float diffuse_weight = kd / (kd + ks); // weight of the diffuse component
    float specular_weight = ks / (kd + ks); // weight of the specular component
    if (get_random_float() < diffuse_weight) {
        is_diffuse = true; // treat as diffuse
    } else {
        is_specular = true; // treat as specular
    }
    ...
    if (get_random_float() < opaque_weight) {
        is_opaque = true; // treat as diffuse
    } else {
        is_transparent = true; // treat as specular
    }

    Vector3f radiance_received = Vector3f(0); // radiance received from the light source


    // ===================================== SPECULAR ====================================

    if (is_specular && is_transparent) {
        // 1) Fresnel mix
        Vector3f I = dir_view_to_hit;              // incoming direction (toward camera)
        float kr   = fresnel(I, normal_hit, material_hit->ior);

        // 2) Randomly pick reflection or refraction
        bool   doReflect = (get_random_float() < kr);
        Vector3f newDir  = doReflect
                            ? reflect(I, normal_hit)
                            : refract(I, normal_hit, material_hit->ior);
        float  pdfDir    = doReflect ? kr : (1.0f - kr);

        // 3) Offset origin and spawn
        Vector3f offset = normal_hit * EPSILON;
        if (!doReflect && dotProduct(newDir, normal_hit) < 0)
            offset = -offset; // going inside
        Ray     nextRay(point_hit + offset, newDir);
        Vector3f Li = castRay(nextRay, depth + 1);

        // 4) Weight by 1/pdf and Russian roulette
        //    delta-BRDF f/pdf = 1, so just 1/pdfDir
        radiance_received = Li;
    }

    if (is_specular && is_opaque) {
        ...
        Vector3f Li = castRay(reflected, depth + 1);
        radiance_received = Li * fresnel(dir_view_to_hit, normal_hit, material_hit->ior);  // energy-scaled by angle
    }
    ...
    if (is_diffuse) {
        ...
        radiance_received = radiance_direct + radiance_indirect;
    ...
    Vector3f radiance_hit_to_view = (radiance_emit + radiance_received) / RussianRoulette; // radiance from the hit point to the camera
    ...
    // if (radiance_received.x == 0 && radiance_received.y == 0 && radiance_received.z == 0){
