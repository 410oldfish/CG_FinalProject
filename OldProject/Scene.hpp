...
int sample_light(const Scene &scene, double u);

/// The probability mass function of the sampling procedure above.
double light_pmf(const Scene &scene, int light_id);
...
inline double get_shadow_epsilon(const Scene &scene) {
    return min(scene.bounds.radius * double(1e-5), double(0.01));
}

inline double get_intersection_epsilon(const Scene &scene) {
    return min(scene.bounds.radius * double(1e-5), double(0.01));
}
