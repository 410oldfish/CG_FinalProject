...
template <>
inline const Mipmap<double> &get_img(const ImageTexture<double> &t, const TexturePool &pool) {
...
T eval_texture_op<T>::operator()(const ImageTexture<T> &t) const {
    ...
    Vector2 local_uv{modulo(uv[0] * t.uscale + t.uoffset, double(1)),
                     modulo(uv[1] * t.vscale + t.voffset, double(1))};
    double scaled_footprint = max(get_width(img), get_height(img)) * max(t.uscale, t.vscale) * footprint;
    double level = log2(max(scaled_footprint, double(1e-8f)));
...
template <typename T>
T eval(const Texture<T> &texture, const Vector2 &uv, double footprint, const TexturePool &pool) {
...
inline ConstantTexture<double> make_constant_float_texture(double f) {
    return ConstantTexture<double>{f};
}
...
inline ImageTexture<double> make_image_float_texture(
        const std::string &texture_name,
        const fs::path &filename,
        TexturePool &pool,
        double uscale = 1,
        ...
        double uoffset = 0,
        double voffset = 0) {
    return ImageTexture<double>{insert_image1(pool, texture_name, filename),
        uscale, vscale, uoffset, voffset};
}
...
inline ImageTexture<double> make_image_float_texture(
        ...
        double uoffset = 0,
        double voffset = 0) {
    return ImageTexture<double>{insert_image1(pool, texture_name, img),
        uscale, vscale, uoffset, voffset};
}
...
inline CheckerboardTexture<Spectrum> make_checkerboard_spectrum_texture(
        ...
        double uoffset = 0, double voffset = 0) {
    return CheckerboardTexture<Spectrum>{
        color0, color1, uscale, vscale, uoffset, voffset};
}

inline CheckerboardTexture<double> make_checkerboard_float_texture(
        double color0, double color1,
        double uscale = 1, double vscale = 1,
        double uoffset = 0, double voffset = 0) {
    return CheckerboardTexture<double>{
        color0, color1, uscale, vscale, uoffset, voffset};
}
